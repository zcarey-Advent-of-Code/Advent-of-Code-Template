...
namespace AdventOfCode.Parsing {
    ...
    public abstract class IParserFilter<TInput, TOutput> : IParser<TInput, IEnumerable<TOutput>> {
    ...
    public sealed class ParserFilter<TInput, TOutput> : IParserFilter<TInput, TOutput> {

        internal ParserFilter(Func<TInput, IEnumerable<TOutput>> filter) {
            this.func = filter;
        }

        private Func<TInput, IEnumerable<TOutput>> func;

        internal override IEnumerable<TOutput> Parse(TInput input) {
            return func.Invoke(input);
        }
